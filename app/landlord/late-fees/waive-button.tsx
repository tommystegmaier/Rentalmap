...
import { markLateFeePaid, waiveLateFee } from './actions';
...
export function MarkLateFeePaidButton({ id }: { id: string }) {
  ...
  async function handle() {
    if (!confirm('Mark this late fee as paid? Use this for fees collected outside the app.')) return;
    setBusy(true);
    try {
  ...
  return (
    <button
      onClick={handle}
      disabled={busy}
      className="rounded border px-2 py-1 text-xs text-muted-foreground transition hover:border-success hover:text-success disabled:opacity-50"
    >
      Mark paid
    </button>
  );
