...
import { signInspection } from './actions';
...
export default async function TenantInspectionDetailPage({
  ...
  const { data: inspData } = await supabase
    .from('inspections')
    .select(
      'id, type, conducted_date, overall_notes, tenant_signed_at, properties:property_id(address)',
    )
    .eq('id', params.id)
    .maybeSingle();
  ...
  async function handleSign() {
    'use server';
    await signInspection(params.id);
    redirect(`/tenant/inspections/${params.id}`);
  }
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isSigned ? (
          <Badge className="border-transparent bg-green-100 text-green-700">
            ...
            Signed {format(parseISO(insp.tenant_signed_at!), 'MMM d, yyyy')}
          </Badge>
        ) : (
          <Badge className="border-transparent bg-yellow-100 text-yellow-700">
        ...
        <Card className="border-dashed">
          <CardContent className="p-4">
            <p className="mb-3 text-sm">
              Please review the inspection below. By signing you confirm that this record
              accurately reflects the condition of the property.
            </p>
            <form action={handleSign}>
              <Button type="submit" className="w-full">
                I agree — this inspection record is accurate
              </Button>
            </form>
          </CardContent>
        </Card>
